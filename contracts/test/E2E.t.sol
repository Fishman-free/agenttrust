...
contract E2ETest is Test {
    function test_fullAcceptedTradeCommitRevealAndPullSettlement() public {
        ...
        vm.prank(buyer); uint256 buyerId = registry.registerAgent("Buyer", "", "");
        vm.prank(seller); uint256 sellerId = registry.registerAgent("Seller", "", "");
        vm.prank(guarantor); uint256 guarantorId = registry.registerAgent("Guarantor", "", "");
        for (uint256 i; i < 3; ++i) {
            vm.deal(jurors[i], 0.1 ether);
            vm.prank(jurors[i]); registry.registerAgent("Juror", "", "");
        }

        vm.prank(buyer); uint256 tradeId = escrow.createTrade(buyerId, sellerId, 1 ether);
        vm.prank(seller); escrow.acceptTrade(tradeId);
        vm.prank(buyer); escrow.fund{value: 1 ether}(tradeId);
        vm.prank(guarantor); escrow.guarantee{value: 1 ether}(tradeId, guarantorId, 1e18, 0);
        ...
        for (uint256 i; i < 3; ++i) {
            ...
            vm.prank(jurors[i]); voting.commitVote{value: 0.1 ether}(caseId, commitment);
        }
        vm.warp(block.timestamp + 1 days);
        for (uint256 i; i < 3; ++i) {
            SchellingVoting.Side side = i < 2 ? SchellingVoting.Side.BUYER : SchellingVoting.Side.SELLER;
            vm.prank(jurors[i]); voting.revealVote(caseId, side, salt);
        }
        vm.warp(block.timestamp + 1 days);
        ...
        assertEq(lost, 1);
        vm.prank(buyer); escrow.withdraw(payable(buyer));
