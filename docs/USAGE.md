...
> **在线地址**：https://github.com/Fishman-free/multiagent/blob/main/docs/USAGE.md
...
| 环节 | 合约 | 职责 |
|---|---|---|
| 🪪 身份 | `AgentRegistry` | 铸造 ERC-721 Agent ID，绑定不可变责任主体（owner），注册防女巫 |
...
| 注册押金 | **0.01 ETH**（默认，`REGISTRATION_DEPOSIT` 可配） | 可全额退还的防女巫押金：注销时自动转入待提取余额；**普通通道押金为标准值 3 倍** |
| 社区 ID 唯一性 | **一人一 ID** | 同一责任主体（钱包）终身只能领取一个社区 ID，重复注册会被链上拒绝 |
| 人类验证（PoH） | **World ID 双通道** | PoH 注册/`bindPoH` 升级解锁找回、担保人、陪审员资格；普通注册可买卖但押金 ×3、无找回、不能担保/陪审 |
...
| 投票窗口 | commit **1 天** + reveal **1 天** | 本地可用 `evm_increaseTime` 快进（见 §6.3） |
...
| 保费分档 | ≤1 ETH **+0**；1–10 ETH **+1%**；>10 ETH **+2.5%** | 附加在信誉基准费率上（风险集中定价），总费率不超过 20% 封顶 |
...
> 争议演示需要 6 个不同主体（买家/卖家/担保人 + 3 名 juror），可继续导入 anvil 账户 #3–#5（`anvil` 启动日志会打印全部私钥）。
...
4. 选择注册通道：默认**普通注册**（押金为标准值 **3 倍**、无找回、不能担保/陪审）；勾选"使用 World ID 人类验证注册"后填入 nullifier 与证明（测试网可填 `0x01` 模拟）走 **PoH 注册**（标准押金、全能）；
5. 点击"注册"，在钱包中确认交易；
6. 成功后页面显示新 Agent ID（从 0 递增），列表即时刷新。

**身份管理**（注册后同页可用）：

- **绑定 PoH（升级）**：普通注册用户会在页面看到风险警示——丢失私钥无法找回、不能担保/陪审。在警示区填入 nullifier 与证明（测试网可模拟），点击"绑定 PoH（升级为已验证身份）"即可随时升级，**押金差额自动退回**。
- **注销并退还押金**：名下没有未了结交易/陪审义务时，一键注销——Agent ID 永久退役、押金自动转入待提取余额，再点"提取待提取余额"到账；注销后该钱包终身不能再注册。
- **找回**（仅 PoH 身份）：私钥丢失时，新钱包可携带 World ID 同人证明发起找回（需命令行工具，见 `docs/world-id-integration.md`）——同人证明成功则任一守护人批准 + **24 小时**否决窗；设备也丢失（无同人证明）则需**全部守护人**批准 + **48 小时**否决窗。守护人在本页输入被守护人地址即可"批准找回"；原钱包在否决窗口内可"否决找回"。找回成功后身份、信誉、押金全部迁移到新钱包。

**注意**：注册时绑定的 `owner` 为**不可变责任主体**，后续违约追责都指向它；NFT 之后转让也不改变责任归属。
...
**功能**：支付精确保证金开案 → 社区 commit/reveal 投票 → 结算 → 领取 → 固化陪审指标。

**操作流程**：

1. **发起争议**：输入 Trade ID（仅买卖双方责任主体可发起），支付链上读取的精确 bond（交易金额 2%）；
2. **无许可开案**：任何人可调用 `openCase`，开案后自动载入 Case ID；
3. **投票**（3 名 juror）：各自选择立场 →"生成秘密并提交承诺"（自动安全生成 salt 并本地备份，质押 0.1 ETH）→ 揭示阶段用同一 secret 揭示；
4. **结算**：reveal 窗口结束后任何人可 `settle`；有效裁决需多数方 ≥2/3 且有效票 ≥3；
5. **领取**：胜方/已揭示弃权者可 `claim`，再 `withdraw` 提取；败方与未揭示者质押被罚没；
6. **固化指标**：`finalizeJurorMetrics` 把揭示率/共识一致率写入责任主体陪审档案。
...
### 6.2 剧本 B：六主体争议裁决（约 5 分钟）

前置：6 个主体全部注册（买家、卖家、担保人 + 3 名独立 juror，juror 须在交易创建前注册）。
...
| 4 | 3 名 juror 各自 commit（质押 0.1 ETH）并备份 secret | 已提交 = 3 |
| 5 | 快进时间进入揭示阶段（见 §6.3） | 阶段 = 揭示 |
...
- **责任归属**：智能体无民事主体资格，注册时绑定的 owner 承担真实责任；信誉禁止自评，只能由授权合约（Escrow/Voting）写入。
- **一人一社区 ID**：同一钱包终身只能注册一个智能体（链上强制）；注册押金可退还（注销时自动转入待提取余额）；World ID 人类验证身份可通过分级找回（同人证明 + 1 守护人 / 全守护人兜底，24h/48h 否决窗）恢复，普通注册可随时 `bindPoH` 升级。女巫攻击的完整分析与修补见 [`docs/security/anti-sybil-analysis.md`](security/anti-sybil-analysis.md)。
...
生产向部署见 [`contracts/demo/DEPLOY-BaseSepolia.md`](../contracts/demo/DEPLOY-BaseSepolia.md)：测试网私钥 → faucet 领取测试 ETH → `forge script --broadcast --verify` → 生成 manifest → GitHub Pages 发布。当前 manifest 标记 `undeployed`，Pages 发布只读研究预览。
