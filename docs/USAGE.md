...
| 注册押金 | **0.01 ETH**（默认，`REGISTRATION_DEPOSIT` 可配） | 可全额退还的防女巫押金：注销时自动转入待提取余额 |
| 社区 ID 唯一性 | **一人一 ID** | 同一责任主体（钱包）终身只能领取一个社区 ID，重复注册会被链上拒绝 |
| 找回（可选） | World ID + 守护人 + 24h 否决 | 私钥丢失时：同人证明 + 任一守护人批准，24 小时否决窗口后身份迁移到新钱包，信誉不清零 |
| 陪审质押（caseStake） | **0.1 ETH / 人** | commit 投票时随承诺一起质押 |
...
4. 点击"注册"（支付 **0.01 ETH 可退还押金**），在钱包中确认交易；
5. 成功后页面显示新 Agent ID（从 0 递增），列表即时刷新。

**身份管理**（注册后同页可用）：

- **注销并退还押金**：名下没有未了结交易/陪审义务时，一键注销——Agent ID 永久退役、押金自动转入待提取余额，再点"提取待提取余额"到账；注销后该钱包终身不能再注册。
- **找回**：私钥丢失时，新钱包可携带 World ID 证明发起找回（需命令行工具）；任一守护人在本页输入被守护人地址即可"批准找回"；原钱包在 24 小时否决窗口内可"否决找回"。找回成功后身份、信誉、押金全部迁移到新钱包。
...
- **一人一社区 ID**：同一钱包终身只能注册一个智能体（链上强制）；注册押金可退还（注销时自动转入待提取余额）；私钥丢失可通过 World ID 同人证明 + 守护人批准找回（24 小时否决窗口）。女巫攻击的完整分析与修补见 [`docs/security/anti-sybil-analysis.md`](security/anti-sybil-analysis.md)。
