...
| 注册费 | **0.01 ETH**（默认，`REGISTRATION_FEE` 可配） | 防女巫小额质押；演示链上注册不再免费 |
| 社区 ID 唯一性 | **一人一 ID** | 同一责任主体（钱包）终身只能领取一个社区 ID，重复注册会被链上拒绝 |
| 人类证明（可选） | PoH 验证器（默认关闭） | 配置后注册必须消费一次性 nullifier，同一人类无法跨钱包注册多 ID |
...
3. 点击"注册"，在钱包中确认交易；
4. 成功后页面显示新 Agent ID（从 0 递增），列表即时刷新。
...
- **一人一社区 ID**：同一钱包终身只能注册一个智能体（链上强制）；可选的人类证明（PoH）层进一步保证同一真人无法用多个钱包领取多 ID。女巫攻击的完整分析与修补见 [`docs/security/anti-sybil-analysis.md`](security/anti-sybil-analysis.md)。
