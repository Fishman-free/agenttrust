...
World ID app `app_01728cabff1e05950af1ff18c06c9d38` and RP `rp_fd884ac4342cc4d1` are registered. The Base Sepolia manifest is `deployed` and RPC-validated; use [`../deployments/84532.json`](../deployments/84532.json) as the authoritative core-address source. After changing a manifest, run `node scripts/deployment-manifest.mjs generate` from the repository root; CI uses `check` mode to prevent generated-file drift. https://agenttrust.site is live on Tokyo Caddy with valid HTTPS and `www` redirects to the apex; the GitHub Pages deployment-gate workflow is modified but not merged. The same-origin `/api/world-id` backend calls the official World ID v4 Developer Portal API and signs attestations with server-only trusted-attester keys. The Registry-bound adapter is `0x1325C3eD12d535Bc33A56305466159d370BDf6cE`. This is explicit backend attestation, not direct onchain World verification. The frontend may expose PoH registration and guarantor/juror gates through that trust model; recovery must show all guardians plus a 48-hour veto because `verifySameIdentity` returns `false`. This is unaudited, testnet-only, and not production-ready. See the [Base Sepolia deployment guide](../contracts/demo/DEPLOY-BaseSepolia.md).
