...
const en = {
  ...
  agents: {
    ...
    verifiedMode: "Register with World ID Proof of Humanity (testnet simulation)", verifiedModeHelp: "Register with World ID Proof of Humanity: unlock recovery, guarantor, and juror eligibility (standard deposit)",
    ...
    depositHelp: "Unverified registration requires 3× the standard deposit and cannot recover an account or serve as guarantor or juror (verification can be added later). Recovery uses same-person proof + 1 guardian (24h veto window), or all guardians (48h veto window).",
    ...
    notVerified: "Proof of Humanity (World ID) is not complete", notVerifiedRisk: "A lost private key cannot be recovered, and this identity cannot serve as a guarantor or juror. Verify soon; the deposit difference is returned after upgrade.",
    bindNullifier: "Bind nullifier (0x… 64 hex digits; any unused value on testnet)", bindProof: "Bind proof (hex; use 0x01 on testnet)", bindInvalid: "Enter a valid nullifier (64 hex digits) and non-empty proof.", bindButton: "Bind PoH (upgrade to verified identity)", bindSuccess: "Humanity proof bound; the deposit difference was returned.",
...
const zhCN: Dictionary = {
  ...
  agents: { title: "智能体注册", subtitle: "以链上 NFT 绑定智能体与责任主体；押金可全额退还。人类验证（World ID）解锁找回、担保与陪审能力。", registryMissing: "当前网络的 AgentRegistry 尚未部署，读取与注册均已禁用。", wrongNetwork: "网络错误：请切换到 {chain}（Chain ID {chainId}）。", currentSubject: "当前责任主体：{address}", name: "智能体名称（如 DataAgent）", description: "能力描述（如：链上数据分析服务）", endpoint: "MCP/A2A 端点（https://…）", guardian1: "守护人 1（必填，紧急联系人地址）", guardian1Aria: "守护人 1（必填）", guardian2: "守护人 2（必填）", guardian3: "守护人 3（可选）", optionalAddress: "0x…（可选）", verifiedMode: "使用 World ID 人类验证注册（测试网模拟）", verifiedModeHelp: "使用 World ID 人类验证注册：解锁找回、担保人、陪审员资格（标准押金）", nullifier: "World ID nullifier（0x…64 位十六进制）", nullifierAria: "World ID nullifier（0x…64 位）", proof: "人类证明（hex，测试网可填 0x01 模拟）", proofAria: "人类证明（hex）", depositHelp: "普通注册押金为标准值 3 倍，且无法找回账号、不能担任担保人或陪审员（可随时升级验证）。找回：同人证明 + 1 守护人（24h 否决窗）；或全守护人批准（48h 否决窗）。", registering: "注册中…", registerDeposit: "注册（押金 {amount} ETH，可退还）", guardiansRequired: "请填写至少两位守护人（紧急联系人）。", guardianInvalid: "守护人地址格式无效。", guardianSelf: "不能把自己设为守护人。", guardianDuplicate: "守护人地址重复。", depositLoading: "注册押金尚未加载。", completeInfo: "请填写完整信息。", validWorldId: "请填写有效的 World ID nullifier 与非空证明。", registered: "注册成功，新 Agent ID：{id}。", missingEvent: "交易已确认，但回执中未找到 AgentRegistered 事件。", identity: "我的社区身份", status: "状态：", active: "活跃", deregistered: "已注销", poh: "人类验证：", verified: "已验证（World ID）", unverified: "未验证", lockedDeposit: "锁定押金：", pending: "待提取余额：", notVerified: "尚未完成人类验证（World ID）", notVerifiedRisk: "丢失私钥将无法找回，且不能担任担保人或陪审员。建议尽快验证升级（升级后退回押金差额）。", bindNullifier: "绑定 nullifier（0x…64 位十六进制，测试网可随意填写未使用值）", bindProof: "绑定证明（hex，测试网可填 0x01 模拟）", bindInvalid: "填写有效的 nullifier（64 位十六进制）与非空证明。", bindButton: "绑定 PoH（升级为已验证身份）", bindSuccess: "已绑定人类证明，押金差额已退回。", activeTrades: "存在未了结的交易（等待对方操作或超时）。", openVotes: "存在未清结的陪审投票义务。", recoveryBlocks: "有进行中的找回请求，先否决或等待过期。", conditions: "条件未满足。", deregisterSuccess: "已注销，押金已转入待提取余额。", deregister: "注销并退还押金", recoveryDeregisterBlock: "有进行中的找回请求，注销暂不可用。", walletCheck: "请确保网络与钱包状态正确。", withdrawDeposit: "提取待提取余额", withdrawSuccess: "押金已提取。", recovery: "找回与守护", recoveryLive: "找回请求进行中：新钱包 {wallet} · 守护人批准 {approvals} / {required} · 路径：{path} · 可执行时间 {date}", samePersonPath: "同人证明（24h 否决窗）", guardianPath: "全守护人兜底（48h 否决窗）", all: "全部", vetoWarning: "若你并未丢失私钥，请在 {hours} 小时否决窗口内立即否决，否则责任主体将被迁移。", veto: "否决找回", vetoSuccess: "已否决找回请求。", noRecovery: "当前没有针对你的找回请求。丢失私钥时，新钱包可携带 World ID 同人证明发起找回（需命令行工具），守护人在下方批准；无同人证明时需全部守护人批准并等待 48 小时否决窗。", approveHelp: "作为守护人：输入被守护人地址并批准其找回请求", protectedAddress: "被守护人地址", approve: "批准找回", approveSuccess: "已批准该找回请求。", registeredAgents: "已注册智能体（{count}）", noAgents: "暂无智能体，注册第一个吧" },
};
